...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'test' and not self.params.do_stereo:
            ...
            left_image_o, right_image_o = self.read_image_random_crop(left_image_path, right_image_path)
    ...
    def read_image_random_crop(self, left_image_path, right_image_path):
        ...
        l_image  = tf.cond(file_cond, lambda: tf.image.decode_jpeg(tf.read_file(left_image_path)), lambda: tf.image.decode_png(tf.read_file(left_image_path)))
        r_image  = tf.cond(file_cond, lambda: tf.image.decode_jpeg(tf.read_file(right_image_path)), lambda: tf.image.decode_png(tf.read_file(right_image_path)))
        ...
        max_offset = tf.cond(crop_cond, lambda: img_height - img_width, lambda: img_width - img_height)
        img_y_offset = tf.cond(crop_cond, lambda: tf.random_uniform([], 0, max_offset, dtype=tf.int32),lambda: 0)
        img_x_offset = tf.cond(crop_cond, lambda: 0, lambda: tf.random_uniform([], 0, max_offset, dtype=tf.int32))

        l_image = tf.cond(crop_cond, lambda: tf.image.crop_to_bounding_box(l_image, img_y_offset, img_x_offset, img_width-1, img_width-1),
                        lambda: tf.image.crop_to_bounding_box(l_image, img_y_offset, img_x_offset, img_height-1, img_height-1))
        l_image  = tf.image.convert_image_dtype(l_image,  tf.float32)
        l_image  = tf.image.resize_images(l_image,  [self.params.height, self.params.width], tf.image.ResizeMethod.AREA)

        r_image = tf.cond(crop_cond, lambda: tf.image.crop_to_bounding_box(r_image, img_y_offset, img_x_offset, img_width-1, img_width-1),
                        lambda: tf.image.crop_to_bounding_box(r_image, img_y_offset, img_x_offset, img_height-1, img_height-1))
        r_image  = tf.image.convert_image_dtype(r_image,  tf.float32)
        r_image  = tf.image.resize_images(r_image,  [self.params.height, self.params.width], tf.image.ResizeMethod.AREA)
