...
class MonodepthModel(object):
    ...
    def get_disp(self, x):
        disp = 0.3 * self.conv(x, 1, 3, 1, tf.nn.sigmoid)  # input , num of layer(ex-2: left_dis/right_dis) , kernel , stride 
        return disp
    ...
    def murge_build_resnet50(self):
        ...
        with tf.variable_scope('left_encoder'):
            left_conv1 = conv(self.left_model_input, 64, 7, 2) # H/2  -   64D
            left_pool1 = self.maxpool(left_conv1,           3) # H/4  -   64D
            left_conv2 = self.resblock(left_pool1,      64, 3) # H/8  -  256D
            left_conv3 = self.resblock(left_conv2,     128, 4) # H/16 -  512D
            left_conv4 = self.resblock(left_conv3,     256, 6) # H/32 - 1024D
            left_conv5 = self.resblock(left_conv4,     512, 3) # H/64 - 2048D

        with tf.variable_scope('left_skips'):
            left_skip1 = left_conv1
            left_skip2 = left_pool1
            left_skip3 = left_conv2
            left_skip4 = left_conv3
            left_skip5 = left_conv4
        
        # DECODING
        with tf.variable_scope('left_decoder'):
            left_upconv6 = upconv(left_conv5,   512, 3, 2) #H/32
            left_concat6 = tf.concat([left_upconv6, left_skip5], 3)
            left_iconv6  = conv(left_concat6,   512, 3, 1)

            left_upconv5 = upconv(left_iconv6, 256, 3, 2) #H/16
            left_concat5 = tf.concat([left_upconv5, left_skip4], 3)
            left_iconv5  = conv(left_concat5,   256, 3, 1)

            left_upconv4 = upconv(left_iconv5,  128, 3, 2) #H/8
            left_concat4 = tf.concat([left_upconv4, left_skip3], 3)
            left_iconv4  = conv(left_concat4,   128, 3, 1)
            self.left_disp4 = self.get_disp(left_iconv4)
            left_udisp4  = self.upsample_nn(self.left_disp4, 2)

            left_upconv3 = upconv(left_iconv4,   64, 3, 2) #H/4
            left_concat3 = tf.concat([left_upconv3, left_skip2, left_udisp4], 3)
            left_iconv3  = conv(left_concat3,    64, 3, 1)
            self.left_disp3 = self.get_disp(left_iconv3)
            left_udisp3  = self.upsample_nn(self.left_disp3, 2)

            left_upconv2 = upconv(left_iconv3,   32, 3, 2) #H/2
            left_concat2 = tf.concat([left_upconv2, left_skip1, left_udisp3], 3)
            left_iconv2  = conv(left_concat2,    32, 3, 1)
            self.left_disp2 = self.get_disp(left_iconv2)
            left_udisp2  = self.upsample_nn(self.left_disp2, 2)

            left_upconv1 = upconv(left_iconv2,  16, 3, 2) #H
            left_concat1 = tf.concat([left_upconv1, left_udisp2], 3)
            left_iconv1  = conv(left_concat1,   16, 3, 1)
            self.left_disp1 = self.get_disp(left_iconv1)

        with tf.variable_scope('rihgt_encoder'):
            right_conv1 = conv(self.right_model_input, 64, 7, 2) # H/2  -   64D
            right_pool1 = self.maxpool(right_conv1,           3) # H/4  -   64D
            right_conv2 = self.resblock(right_pool1,      64, 3) # H/8  -  256D
            right_conv3 = self.resblock(right_conv2,     128, 4) # H/16 -  512D
            right_conv4 = self.resblock(right_conv3,     256, 6) # H/32 - 1024D
            right_conv5 = self.resblock(right_conv4,     512, 3) # H/64 - 2048D

        with tf.variable_scope('right_skips'):
            right_skip1 = right_conv1
            right_skip2 = right_pool1
            right_skip3 = right_conv2
            right_skip4 = right_conv3
            right_skip5 = right_conv4
        
        # DECODING
        with tf.variable_scope('right_decoder'):
            right_upconv6 = upconv(right_conv5,   512, 3, 2) #H/32
            right_concat6 = tf.concat([right_upconv6, right_skip5], 3)
            right_iconv6  = conv(right_concat6,   512, 3, 1)

            right_upconv5 = upconv(right_iconv6, 256, 3, 2) #H/16
            right_concat5 = tf.concat([right_upconv5, right_skip4], 3)
            right_iconv5  = conv(right_concat5,   256, 3, 1)

            right_upconv4 = upconv(right_iconv5,  128, 3, 2) #H/8
            right_concat4 = tf.concat([right_upconv4, right_skip3], 3)
            right_iconv4  = conv(right_concat4,   128, 3, 1)
            self.right_disp4 = self.get_disp(right_iconv4)
            right_udisp4  = self.upsample_nn(self.right_disp4, 2)

            right_upconv3 = upconv(right_iconv4,   64, 3, 2) #H/4
            right_concat3 = tf.concat([right_upconv3, right_skip2, right_udisp4], 3)
            right_iconv3  = conv(right_concat3,    64, 3, 1)
            self.right_disp3 = self.get_disp(right_iconv3)
            right_udisp3  = self.upsample_nn(self.right_disp3, 2)

            right_upconv2 = upconv(right_iconv3,   32, 3, 2) #H/2
            right_concat2 = tf.concat([right_upconv2, right_skip1, right_udisp3], 3)
            right_iconv2  = conv(right_concat2,    32, 3, 1)
            self.right_disp2 = self.get_disp(right_iconv2)
            right_udisp2  = self.upsample_nn(self.right_disp2, 2)

            right_upconv1 = upconv(right_iconv2,  16, 3, 2) #H
            right_concat1 = tf.concat([right_upconv1, right_udisp2], 3)
            right_iconv1  = conv(right_concat1,   16, 3, 1)
            self.right_disp1 = self.get_disp(right_iconv1)

    def left_build_resnet50(self):
        #set convenience functions
        conv   = self.conv
        if self.params.use_deconv:
            upconv = self.deconv
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.elu):
            with tf.variable_scope('model', reuse=self.reuse_variables):
                ...
                if self.params.do_stereo:
                    ...
                    #self.right_model_input = self.right
                    self.left_model_input = self.left
                    self.right_model_input = self.right
                ...
                if self.params.encoder == 'vgg':
                    ...
                    self.left_build_resnet50()
                    self.right_build_resnet50()
                else:
                    return None
    ...
    def build_outputs(self):
        ...
        with tf.variable_scope('disparities'):
            self.left_disp_est  = [self.left_disp1, self.left_disp2, self.left_disp3, self.left_disp4]
            self.right_disp_est  = [self.right_disp1, self.right_disp2, self.right_disp3, self.right_disp4]
            self.disp_left_est  = [tf.expand_dims(d[:,:,:,0], 3) for d in self.left_disp_est]
            self.disp_right_est = [tf.expand_dims(d[:,:,:,0], 3) for d in self.right_disp_est]

          #  self.disp_est  = [self.right_disp1, self.right_disp2, self.right_disp3, self.right_disp4]
